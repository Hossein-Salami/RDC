...
N = num(3);                              % number of spatial nodes in the bead 
                                         % ----------------------------------                                        
drug = 'CEX';                            % 'CEX' or 'AMP'
carrier = 'immobead';                     % carrier TYPE (NOT SIZE) - 'agarose' or 'immobead' 
pH_control = 'y';                        % control pH? ('y' or 'n')
v_reactor = 50e-3;                      % [Lit] reactor volume
enzyme_mass = 10e-3;                     % total [gr] enzyme  
loading = 26e-3;                         % gr enzyme / gr wet bead 
seed_percent = 0.00;                     % percent of seeding (% wt)
tfinal = 120.0;                           % simulation time in minutes
pH_set = 7;                            % bulk pH control / initial
s0 = 200e-3;                             % substrate [M] @t=0
n0 = 150e-3;                             % nucleophile [M] @t=0
                                         % ----------------------------------
